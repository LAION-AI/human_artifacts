...
from dataset import HumanArtifact_dataloader
...
# train and test is same for now! Divide when data is bigger.
train_dataloader = DataLoader(train_dataset, batch_size=2, shuffle=True, num_workers=8)
test_dataloader = DataLoader(train_dataset, batch_size=1, shuffle=False, num_workers=8)
...
def train(model, epoch):
    ...
    for batch_idx, data in enumerate(train_dataloader):
        data1, target = data["image"].to(DEVICE), data["mask"].to(DEVICE)
